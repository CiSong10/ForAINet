...
class ForestryAnalyzer:
    ...
    def process_all_plots(self, file_paths: List[str], input_dir: str, output_dir: str, test_file_paths: Optional[List[str]] = None, field_file_path: Optional[str] = None) -> None:
    ...
    def _process_single_plot(self, plot_idx: int, file_path: str, input_dir: str, output_dir: Path, test_file_paths: Optional[List[str]] = None, field_file_path: Optional[str] = None) -> None:
        self.logger.info(f"Processing plot {plot_idx}")

        plot_suffix = Path(file_path).stem.split("_")[-1]
        plot_dir = output_dir / f'plot_{plot_suffix}'
    ...
    def _extract_ply_data(self, sem_data: Any, ins_data: Any) -> Tuple[Dict[str, np.ndarray], Dict[str, np.ndarray]]:
    ...
    def _load_extract_las_data(self,  input_dir: str, filename: str):
        ...
        coords = {'x': np.array(las.x),
                  'y': np.array(las.y),
                  'z': np.array(las.z),}
        
        labels ={'pred_sem': las['PredSemantic'],
                 'pred_ins': las['PredInstance'],}

        return coords, labels

    def _generate_dtms(self, coords: Dict, labels: Dict, plot_dir: Path, plot_suffix: str) -> Tuple[np.ndarray, np.ndarray]:
        ...
        output_DTM_as_pc(dtm_pred, plot_dir / f'floor_pre_pynn_{plot_suffix}.ply')
        
        return None, dtm_pred
    ...
    def _calculate_plot_metrics(self, coords: Dict, labels: Dict, dtm_gt: np.ndarray, dtm_pred: np.ndarray) -> PlotMetrics:
        ...
        tree_coords = np.column_stack([coords['x'][idx_pred_tree], coords['y'][idx_pred_tree]])
        
        if len(tree_coords) > 3:
            hull = ConvexHull(tree_coords)
            convex_area = hull.area
            unique_trees = len(np.unique(labels['pred_ins'][idx_pred_tree]))
            stem_density = (unique_trees / convex_area) * 10000  # trees per hectare
        else:
            stem_density = None
        
        return PlotMetrics(stem_density=stem_density,)
    ...
    def _process_tree_instances(self, coords: Dict, labels: Dict, tree_mask: np.ndarray, 
        ...
        # for i in tqdm(range(len(unique_instances)), total=len(unique_instances), desc="trees"):
        #     try: 
        #         instance_id, metrics = process_single_tree(args_tuple=args_list[i])
        #     except RuntimeError as e:
        #         self.logger.warning(f"Exception occurred while processing {tree_type} tree {instance_id}: {str(e)}")
        #         continue
        #     if metrics:
        #         tree_metrics[instance_id] = metrics
    ...
    def _save_results(self, plot_dir: Path, gt_trees: Dict, pred_trees: Dict, 
        ...
        self._save_plot_results(plot_dir, plot_metrics, tree_metrics, 0, len(pred_trees))
    ...
    def _save_tree_results(self, plot_dir: Path, gt_trees: Dict, pred_trees: Dict, metrics: Dict) -> None:
        ...
        for pred_id, pred_tree in pred_trees.items():
            row = {
                'pred_id': pred_id,
                'pred_height': pred_tree.height,
                'pred_dbh': pred_tree.trunk_diameter,
                'pred_crown_diameter': pred_tree.crown_diameter,
                'pred_crown_volume': pred_tree.crown_volume,
                'pred_x_location': pred_tree.x_location,
                'pred_y_location': pred_tree.y_location
            }
            tree_data.append(row)
        
        df = pd.DataFrame(tree_data).sort_values('pred_id')
        
        # summary_stats = self._calculate_summary_statistics(metrics)
        # summary_df = pd.DataFrame([summary_stats])

        df.to_csv(plot_dir / 'tree_analysis_results.csv', index=False)
    ...
    def _save_plot_results(self, plot_dir: Path, plot_metrics: PlotMetrics, 
        ...
        n_matched = None # len([errors for errors in tree_metrics['height_errors']])
        detection_rate = None # (n_pred_trees / n_gt_trees * 100) if n_gt_trees > 0 else 0
        detection_accuracy = None # (n_matched / n_gt_trees * 100) if n_gt_trees > 0 else 0
        
        plot_results = {
            'dtm_coverage': plot_metrics.dtm_coverage,
            'dtm_rmse': plot_metrics.dtm_rmse,
            'stem_density': plot_metrics.stem_density,
            'n_reference_trees': None, # n_gt_trees,
            'n_detected_trees': n_pred_trees,
            'n_matched_trees': n_matched,
            'detection_rate': detection_rate,
            'detection_accuracy': detection_accuracy,
            'omission_error':  None, # 100 - detection_accuracy,
            'commission_error':  None, # ((n_pred_trees - n_matched) / n_pred_trees * 100) if n_pred_trees > 0 else 0
        }
